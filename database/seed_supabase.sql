-- ============================================================================
-- TERANGA PALACE — Données de démonstration, COMPATIBLES avec supabase_schema.sql
-- (celui-ci utilise des colonnes camelCase entre guillemets : "numeroChambre",
-- "prixParNuit", etc. — contrairement à database/seed.sql, qui est un ancien
-- fichier écrit pour une variante sans guillemets et une table "utilisateurs"
-- au lieu de "users". Utilise TOUJOURS ce fichier-ci avec supabase_schema.sql.)
--
-- Ordre d'exécution recommandé dans Supabase → SQL Editor :
--   1. database/supabase_schema.sql   (recrée les tables, vide tout)
--   2. database/seed_supabase.sql     (ce fichier)
--   3. database/enable_realtime.sql   (déjà fait normalement, sans risque à relancer)
-- ============================================================================
...
-- -------- UTILISATEURS (comptes de démonstration) : table "users", pas "utilisateurs" --------
INSERT INTO users (email, password, role, "idClient") VALUES
